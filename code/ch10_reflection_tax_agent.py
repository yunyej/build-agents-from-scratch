"""Chapter 10 — Reflection / critique after planning + execution (tax demo).

Phase 0: OpenAI JSON session facts (same as Ch 9).
Phase A: JSON plan. Phase B: tool loop with a recorded trace.
Phase C: JSON reflection — evidence, tool honesty, completeness — then final_answer.
...
def run_execution_phase_with_trace(
    ...
    """Same tool loop as Ch 9, but append each tool call + output to `tool_trace`.

    Returns (draft_answer, tool_trace, execution_llm_rounds).
    """
    ...
    system = (
        ...
        "If the user asks a 'what if' scenario (e.g. extra 401(k) contribution), say honestly what the stub tools can "
...
def _reflection_fallback(draft: str, err: str) -> dict[str, Any]:
    return {
        ...
        "completeness_notes": "",
        "issues_found": [f"reflection_parse_error: {err}"],
        "final_answer": draft,
    }
...
def run_reflection_phase(
    ...
    """
    Second pass: evidence alignment, numeric/tool honesty, completeness vs user goals.
    Returns JSON including issues_found and final_answer (revised if needed).
    """
    ...
    try:
        ...
        return _reflection_fallback(draft_answer, str(e))
...
def main() -> None:
    ...
    parser.add_argument("--no-reflect", action="store_true", help="skip reflection; print draft answer only")
    ...
    if not args.no_plan:
        ...
        print("\n=== Phase B: Execution (tools + trace) ===\n")
    else:
        print("\n=== Execution (tools + trace, no plan) ===\n")

    draft, trace, _rounds = run_execution_phase_with_trace(client, user_text, plan, session_facts=facts)
    print("--- Tool trace ---")
    print(json.dumps(trace, ensure_ascii=False, indent=2))
    print("\n--- Draft answer (pre-reflection) ---\n")
    print(draft)

    if args.no_reflect:
        return

    print("\n=== Phase C: Reflection (JSON) ===\n")
    ...
    print(reflection.get("final_answer", draft))
